...
class HapticGlove:
    ...
    MOTORS = np.array([np.array([0, 0, 1]), np.array(
        [0, 0, -1]), np.array([0, -1, 0]), np.array([0, 1, 0])])  # array of motor positions
    TIMEOUT = 10  # seconds
    ...
    def send_push_feedback(self,
                           message: np.ndarray
                           ) -> None:
        for _ in range(0, 10):
            self.socket.send(f'{message}\n'.encode('ascii'))

    def send_pull_feedback(self,
    ...
    def stop_feedback(self) -> None:
        for _ in range(0, 10):
            self.socket.send(
                f'{self.MINIMUM_INTENSITY_MESSAGE}\n'.encode('ascii'))
    ...
    def find_intensity_array(self,
        ...
        for i in range(0, len(motor_positions)):
            motor_distance[i] = self.find_distance(
                U, motor_positions[i], normalized=True)
            mapped[i] = self.reverse_map_to_range(
                motor_distance[i], 0.0, math.sqrt(2), 1, .59, bounded=True)
